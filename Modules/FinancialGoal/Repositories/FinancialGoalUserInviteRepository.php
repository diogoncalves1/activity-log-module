...
class FinancialGoalUserInviteRepository
{
    ...
    protected FinancialGoalActivityRepository $activityRepo;

    public function __construct(FinancialGoalRepository $financialGoalRepository, SharedRoleRepository $sharedRoleRepository, FriendshipRepository $friendshipRepository, FinancialGoalUserRepository $financialGoalUserRepo, FinancialGoalActivityRepository $activityRepo)
    {
    ...
    public function invite(Request $request, string $id, string $userId)
    {
        return DB::transaction(function () use ($request, $id, $userId) {
            ...
            $this->activityRepo->storeActivity($input['financial_goal_id'], $user->id, 'user_invited', ['invitedUserId' => $userId, 'role' => $sharedRoleInvite->code, 'invitedUserName' => $invite->receiver->name, 'inviteId' => $invite->id]);
